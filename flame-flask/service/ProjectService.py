...
class ProjectService:
    ...
    def all_project():
        ...
        for project in projects:
            query = db.session.query(User).join(ProjectUser, User.user_id == ProjectUser.user_id).filter(ProjectUser.project_id == project.project_id)
            print('-'*80)
            print(str(query.statement)) # 对应 sql
            print('-'*80)
            users: list[User] = query.all()
            user_identities: list[UserIdentity] = [UserIdentity(phone=user.phone, user_id=user.user_id) for user in users]
    ...
    def get_project_by_project_id(project_id: int) -> dict | None:
        ...
        users: list[User] = db.session.query(User).join(ProjectUser, User.user_id == ProjectUser.user_id).filter(ProjectUser.project_id == project.project_id).all()
    ...
    def get_project_by_user_id(user_id: str) -> list | None:
        project_ids: list[int] = [pu.project_id for pu in ProjectUser.query.filter_by(user_id=user_id).all()]
        projects: list[Project] = Project.query.filter(Project.project_id.in_(project_ids)).all()
        project_list: list[dict[str, Any]] = []
