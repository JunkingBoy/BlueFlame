...
class UserRegisterDTO(BaseDTO):
    ...
    def check_phone_length(cls, v):
        if not isinstance(v, str):
            raise ValueError(f'Phone must be a string')
        if len(v) != 11:
            raise ValueError(f'Phone number must be exactly 11 digits long')
        return v
    ...
    def check_password_length(cls, v):
        ...
        if len(v) < 6 or len(v) > 16:
            raise ValueError(f'Password must be between 6 and 128 characters long')
        return v

    @field_validator('password_confirm')
    def check_password_confirm_length(cls, v):
        if not isinstance(v, str):
            raise ValueError(f'Password confirmation must be a string')
        if len(v) < 6 or len(v) > 16:
            raise ValueError(f'Password confirmation must be between 6 and 128 characters long')
        return v
...
class UserModifyPasswordDTO(BaseDTO):
    ...
    def check_password_length(cls, v: str):
        if len(v) < 6 or len(v) > 16:
            raise ValueError(f'password must be between 6 and 16 characters long')
        return v
    
    @field_validator('new_password')
    def check_new_password_length(cls, v: str):
        if len(v) < 6 or len(v) > 16:
            raise ValueError(f'new password must be between 6 and 16 characters long')
        return v
...
class UserModifyNameDTO(BaseDTO):
    ...
    def check_name_length(cls, v: str):
        if len(v) < 1 or len(v) > 16:
            raise ValueError(f'name must be between1 and 16 characters long')
        if re.fullmatch(r"[^\w\s]", v):  # \w 匹配任何字母数字字符，\s 匹配空格
            raise ValueError(f'name cannot consist solely of special characters')
        return v
